...
bool            CPipeStatusServer::m_bDestroyed = FALSE;  
CPipeStatusServer*  CPipeStatusServer::m_pInstance = NULL;

CPipeStatusServer::CPipeStatusServer(void)
  : m_hThread(nullptr)
{
}
...
CPipeStatusServer* CPipeStatusServer::GetInstance(void)
{
  if (m_bDestroyed)
  {
    m_pInstance = new CPipeStatusServer();
    atexit(Kill);
    m_bDestroyed = false;
  }
...
void __stdcall OnWaitTimerCallback_Status(PVOID lpParameter, BOOLEAN bTimerOrWaitFired)
{
  PipeInfo* pi = (PipeInfo *)lpParameter;
  pi->bConnected = TRUE;
}

BOOL CPipeStatusServer::Initialize(const CString& strName)
{
  if (strName.IsEmpty())
    return FALSE;
  else
...
void CPipeStatusServer::Finalize()
{
  PipeInfo* pPI = nullptr;
...
void CPipeStatusServer::TransmitMessage(const CString& strLog)
{
  BOOL bRet = FALSE;
  ULONG ulProcessID = 0;
  PipeInfo* pPI = nullptr;
...
void CPipeStatusServer::TransmitMessage(LPCTSTR pszFormat, ...)
{
  va_list   ap;
...
BOOL CPipeStatusServer::IsConnected(void)
{
